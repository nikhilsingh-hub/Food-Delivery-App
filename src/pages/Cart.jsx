...
function Cart() {
    ...
    return (
        <div>
            <Link to="/" className="fixed top-20 text-lg hover:text-gray-100 hover:scale-110 transition duration-300 flex w-fit bg-[#FCDE58] text-white font-bold py-2 px-4 rounded"><ArrowleftSvg /> Add more items</Link>
            {allCartProducts.length > 0 ?
                <div className='bg-white w-full h-screen mt-14 flex items-center justify-center'>
                    <div className='bg-white text-slate-900 w-2/3 rounded-lg shadow-md mt-2 mx-auto border border-gray-200 sm:p-6 lg:p-8'>
                        <Tabs list={allTabs} activeTab={currentTab} onTabSwitch={handleSwitch} />
                        <div className={`${currentTab !== 'Summary' ? 'hidden' : ''}`}>
                            <ProductSummary cart = {allCartProducts}/>
                ...
                : <div className='mt-32 text-center'>No added product</div>}
        </div>
    )
}
